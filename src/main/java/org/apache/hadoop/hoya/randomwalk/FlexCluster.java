...
public class FlexCluster extends Test {
  private static final Logger log = LoggerFactory.getLogger(FlexCluster.class);
  ...
  protected List<String> getCommand(State state) {
    RandomwalkHoyaConfiguration conf = new RandomwalkHoyaConfiguration(FlexCluster.class, state);

    List<String> command = Lists.newArrayList(conf.getHoyaCommand());

    command.add(HoyaActions.ACTION_FLEX);
