 ...
 * 
 */
public class DestroyClusterTest {
  ...
  public void testSuccessfulCommand() {
    ...
    DestroyCluster destroy = new DestroyCluster();

    State state = new State(props, new InMemoryErrorReporter());

    List<String> command = destroy.getCommand(state);

    List<String> expected = Arrays.asList("/usr/bin/hoya", HoyaActions.ACTION_DESTROY, "cluster", Arguments.ARG_MANAGER + " localhost:1234",
