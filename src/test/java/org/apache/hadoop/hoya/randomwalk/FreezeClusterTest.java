 ...
 * 
 */
public class FreezeClusterTest {
  ...
  public void testSuccessfulCommand() {
    ...
    FreezeCluster freeze = new FreezeCluster();

    State state = new State(props, new InMemoryErrorReporter());

    List<String> command = freeze.getCommand(state);

    List<String> expected = Arrays.asList("/usr/bin/hoya", HoyaActions.ACTION_FREEZE, "cluster", Arguments.ARG_MANAGER + " localhost:1234",
